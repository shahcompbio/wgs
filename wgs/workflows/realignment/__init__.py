...
def realign_bam_files(
    ...
    workflow.subworkflow(
        ...
        args=(
            mgd.TempInputFile("inputdata_read1.fastq.gz", "readgroup", axes_origin=[]),
            mgd.TempInputFile("inputdata_read2.fastq.gz", "readgroup", axes_origin=[]),
            mgd.OutputFile(output, extensions=['.bai']),
            mgd.OutputFile(metrics_output, extensions=['.yaml']),
            mgd.OutputFile(metrics_tar),
            mgd.OutputFile(output_tdf),
